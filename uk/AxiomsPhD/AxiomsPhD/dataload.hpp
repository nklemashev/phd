...
namespace dataload
{
	dt::tradestat loadTradeStat(const std::string& pfName, const std::string& xfName, 
		const std::string& tfName, const std::string& nfName);
	...
	std::vector<axioms::group> splitStrataIntoClasses(const std::vector<dt::matrix>& Xs, 
		...
		const std::vector<std::vector<int>>& splits);
	dt::strata splitStrataIntoClasses(const std::vector<dt::matrix>& Xs, 
		const dt::matrix& P, const std::vector<std::string> tNames, const std::vector<std::string>& cNames,
		const std::vector<dt::intervals>& splits);
	...
	std::vector<dt::intervals> loadClassSplitDataNew(const std::string& fName);
	dt::splitData loadClassSplitDataNew2(const std::string& fName);
	dt::lorenzData loadLorenzCurveData(const std::string& fName);
}
